...
{
    public partial class CallPlanByGovHospital : BasePage
    {
...
//										", Session[Constants.SESSION_GOV_HOS_DATE].ToString(), scCode);
            ...
            string query = string.Format(@"SELECT A.SLSMN,A.SMDESC, A.PDTCDE,A.[$JNMAH],[$JNMAR],
                                        A.[$JNMAD],A.[$JNMAQ], A.[$JNMAF],A.[$JNMAI], A.CUST, 
                                        A.CNAME, A.CMGRP2, A.CADD1, A.CADD2,A.ZCMTYP, A.[$JNMAE],
                                        A.[$JNMAO],A.[$JNMAP],A.[$JNMAG], B.ZLVL2 as ZLVL,CPLDTE
                                        FROM zsa756pf a left join zlvl b on a.cmgrp2=b.cmgrp2 and           
