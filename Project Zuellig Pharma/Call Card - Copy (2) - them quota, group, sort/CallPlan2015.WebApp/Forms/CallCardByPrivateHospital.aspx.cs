...
{
    public partial class CallCardByPrivateHospital : BasePage
    {
        ...
        private void LoadAllPharmacy()
        {
            ...
            ltSumAve6LastMonth.Text = data.Sum(item => item.Ave6LastMonthValue).ToString("n1");
            ltSumMtdAllSources.Text = data.Sum(item => item.MtdAllSourcesValue).ToString("n1");
            ltSumMtdScSources.Text = data.Sum(item => item.MtdScSourcesValue).ToString("n1");
        }
...
//                                            A.IMDESC, A.$JNQAD, A.$JNQAE, A.$JNQAF, A.$JNQAA, A.$JNQAB,        
//                                            A.$JNQAC, value(B.IMSOH,0) as IMSOH
//                                            FROM zsa758pf a left join zcmimsoh b on 
...
//                                            ", custCode, Session[Constants.SESSION_USER_NAME]);
            ...
            string query = string.Format(@"SELECT A.CUST, A.CNAME, A.CADD1, A.CMCONT, A.CMPHON, A.ARTRM,      
                                        A.CMGRP2, A.PDTCDE, A.SLSMN, A.SMDESC, A.ZCITYP, A.PRNCPL, A.ITEM, 
                                        A.IMDESC, A.[$JNQAD], A.[$JNQAE], A.[$JNQAF], A.[$JNQAA], A.[$JNQAB],A.[$JNQAC], B.IMSOH as IMSOH
                                        FROM zsa758pf a left join zcmimsoh b on 
                                        a.cust=b.cust and a.item=b.item 
            ...
            return CallPlanUtility.ConvertPrivateHospitalTableToList(table);
        }
        ...
        private List<PharmacyCustomerData> CreateDataForBinding(List<PharmacyCustomerData> data)
        {
            ...
            var grouppedData = data.Where(item => item.BestSaleFlag == "B")
                                   ...
                                   .ToDictionary(item => item.Key, item => item.OrderByDescending(code => code.Ave6LastMonthValue).ToList());
        ...
        private void InitMasterData()
        {
            ...
            lbladdress.InnerText = sData.CustomerAddress;
            lblTelephone.InnerText = sData.PhoneNumber;
            lblContacPerson.InnerText = sData.ContactName;
