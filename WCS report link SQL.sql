-	link server: link server from [192.168.17.230] to WCS
-	tạo table luu ket qua chạy câu lệnh sql trên [192.168.17.230]: 
-	lập lịch chạy t3->t7 trên Zeris: 192.168.17.230 để goi cau lenh sql luu ket qua vao cac bang moi dem
-	chạy sql ra 5 sheet 
-	gửi file kết quả qua email cho helpdesk từ Zeris: 192.168.17.230

http://www.purplefrogsystems.com/blog/2014/04/excel-doesnt-open-csv-files-correctly-from-sp_send_dbmail/

https://www.google.com.vn/webhp?sourceid=chrome-instant&ion=1&espv=2&ie=UTF-8#q=attached+file+email+csv+multiple+sheet+using+sql+server

https://blogs.msdn.microsoft.com/sqlagent/2010/11/10/sql-database-mail-send-emails-with-attachment/
=========================================================
USE [Zeris]
GO
/****** Object:  StoredProcedure [dbo].[Report_WCS]    Script Date: 1/12/2017 1:53:11 PM ******/
SET ANSI_NULLS ON
GO
...
order by
    ...
    @recipients = 'helpdesk@zuelligpharma.com.vn',
    @query = @sqlqry,
...
select
	...
	cd_time as [Time]
    --,m.* 
	into [Zeris].[dbo].WCS_RoutingHistory
from 
    [LINK_TO_WCS].[RouteDirector].[dbo].cd_carvisits_map m join [LINK_TO_WCS].[RouteDirector].[dbo].cd_carrier c on (c.cd_carrier_key = m.cd_carrier_fk)
                left join [LINK_TO_WCS].[RouteDirector].[dbo].cd_node n on (m.cd_node_fk = n.cd_node_key)
where
...
select
    ...
    SUM(case when ct.rp_con_type_id = 'BAG'  then 1 else 0 end) as BAG,
	count(distinct w.rp_work_tk) as WorkAssignments
	into [Zeris].[dbo].WCS_CartonCountQuery
from
    [LINK_TO_WCS].[PickDirector].[dbo].rp_work w join [LINK_TO_WCS].[PickDirector].[dbo].rp_allocation a on (w.rp_work_tk = a.rp_work_ftk)
            join [LINK_TO_WCS].[PickDirector].[dbo].rp_container c on (a.rp_container_ftk = c.rp_container_tk)
            join [LINK_TO_WCS].[PickDirector].[dbo].rp_con_type ct on (c.rp_con_type_ftk = ct.rp_con_type_tk)
...
select 
    s.rp_timestamp, 
    s.rp_dest_container 
	into [Zeris].[dbo].WCS_ContainerQA
from 
    [LINK_TO_WCS].[PickDirector].[dbo].[rp_special_event] s 
where s.rp_event_type = 'LOG_QA'
and s.rp_operator_id is not null
...
SELECT
	...
	into [Zeris].[dbo].WCS_SC_WA_Count
...
select
	...
	f.rp_zone_id,
	count(*) as Case_Count
	into [Zeris].[dbo].WCS_FC_Zone_Container_Count
from
	FCWorks f
group by
	DATEADD(day, DATEDIFF(day, 0, rp_received), 0),
